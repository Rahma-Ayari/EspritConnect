...
public class ChatbotAiService {
    ...
    public String buildFallbackReply(String userMessage, List<FAQ> matchedFaqs) {
        ...
        if (isConfigured()) {
            return "The AI service is temporarily busy (rate limit). Please try again in a minute. "
                    + "You can still ask about ESPRIT campus location, careers, CV tips, jobs, events, or mentoring — "
                    + "I'll answer from built-in help when I can.";
        }

        return "I'd love to help with that! Add your Gemini API key to "
